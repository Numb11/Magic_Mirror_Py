...
def Send_Pi_Data(UL,UR,BCENT,CENT, User_Name,TbleRmdInfo,name): #Function called to send data to the mirror, it requires the widgets names being used to be passed in as arguments
    ...
    if Pi_WifiStatus == False: #Selection to determine if the Wifi info needs to be sent
        while Wifi_User == "" and Wifi_Pass == "": #Waiting on user input in a seperate thread pertatining to the wifi information gathering window
            pass
    else: #If not, the pi is connected and Wifi og-in doesn't need to be sent

        try: #Requiered to preve4nt thread crashing
            ...
            return #Required to exit out of the thread
        except:
            BlueSock.close() #Close the socket to avoid rescource leak
            Data_Sent_Stat = False #Update Data_Sent_Stat so the UI cna be updated appropriately
            return #Required to exit out of the thread
...
class WifiSelection(CTkToplevel): # Creation of another window used for the the inputting of wifi credentials
    ...
    def get_password(self,SSID): #Method used to get the wifi password and update the UI accordingly
        ...
        if NetworkPass == None: #If a network password easnt bale to be fetched
            ...
            self.Button = CTkButton(self, text = "Continue with Bluetooth", command = self.ContinueBlue) #Create a button widget offfering the option to continue with bluetooth communication
            self.Button.place(x=150, y=150) #Place the widget in the screen coordinates specified
    ...
    def ContinueBlue(self): #This method is called when the continue with bluetooth button is pressed
...
# ----- This method has the same functionality as the one above but for the opposite side -----
    ...
    def Send_Widget_Data(self):
        ...
        Send_Data_Thread.join() #Wait for thread to finish
